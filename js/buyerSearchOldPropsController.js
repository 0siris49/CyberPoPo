...
export class buyerSearchOldPropsController{
        ...
        if (searchButton) {
            searchButton.addEventListener("click", async function fetchDisplayProperties (event) {
                ...
                if(email != ""){
                    ...
                    if(propStatus == "Unavailable"){
                        console.log("This shit old buddy");
                        let searchParamsObj = new searchParams(propLocation,propPriceRange,propType,propBedrooms,propStatus);
                        let initBuyerSearchOldPropsController = new buyerSearchOldPropsController();
                        var FinalSearchResult = await initBuyerSearchOldPropsController.passSearchParams(searchParamsObj);
                        initBuyerSearchOldPropsController.displaySearchResultsController(FinalSearchResult,email);
