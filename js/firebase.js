...
export default class FirebaseClass {
    ...
    async checkEmailPassType(arg) {
        ...
        if (preReqResponse == "") {
            var errorMessage = "No such User/User is suspended."
            return errorMessage;
        } else {
            if (responseToString == "") {
        ...
        try {
            ...
            if (responseToStringq1 != "") {
                if (responseToStringq2 != "") {
                    ...
                    var message = "No such REA Email";
                    return message;
                    
                }
            } else {
                var message = "No such Seller Email";
                return message;
            }
            return null;
        } catch (e) {
    ...
    async updatePropListDetails(arg) {
        ...
        if (responseToStringq2 != "") {
            ...
            if (responseToStringq1 != "") {
                ...
                location.reload();
                return("Property listing details updated");
            } else {
    ...
    async submitRating(arg) {
        ...
        if (sellerEmail != null) {
            ...
            if (responseToStringq1 != "") {
                ...
                var message = "You cannot review an REA you have not worked with before!!!";
                return message;
    ...
    async shortlistProp(propID, currentEmail) {
        ...
        if (responseToStringq1 == "") {
            ...
            return("Property successfully shortlisted!");

        } else {
            
            return("You have already shortlisted this property before!");
        }
    ...
    async updateUser(userObj) {
        if (userObj.email != "") {
            ...
            if (responseToString == "") {
                ...
                var reviewOverview = [];
                var reviewerTitle = [];
                var stringArr = allDataA.split("---");
                var totalReview = stringArr.length - 1;
    ...
    async suspendUser(userToSuspend) {
        const q = query(collection(db, "CSIT314/All-Users/UserData"), where("Email", "==", userToSuspend));
        const updateRef = doc(db, "CSIT314/All-Users/UserData", userToSuspend);
        const queryAns = await getDocs(q);
        var response = "";
        queryAns.forEach((doc) => {
            response += doc.id;
    ...
    async createUser(newUserObj) {
        const q = query(collection(db, "CSIT314/All-Users/UserData"), where("Email", "==", newUserObj.email));
        ...
        if (responseToString == "") {
            if (newUserObj.userType === "REA") {
                try {
                    const userData = collection(db, "CSIT314/All-Users/UserData");
                    await setDoc(doc(userData, newUserObj.email), {
                        FirstName: newUserObj.fN,
                        LastName: newUserObj.lN,
            ...
            } else if (newUserObj.userType === "buyer") {
                try {
                    const userData = collection(db, "CSIT314/All-Users/UserData");
                    await setDoc(doc(userData, newUserObj.email), {
                        FirstName: newUserObj.fN,
            ...
            } else if (newUserObj.userType === "seller") {
                try {
                    const userData = collection(db, "CSIT314/All-Users/UserData");
                    await setDoc(doc(userData, newUserObj.email), {
                        FirstName: newUserObj.fN,
    ...
    async getProfileList() {
        const querySnapshot = await getDocs(collection(db, "CSIT314/All-Users/UserTypes"));
        var allProfiles = [];
        var allStatus = [];
        querySnapshot.forEach(async (doc) => {
            ...
            allProfiles.push(id);
            var docStringify = JSON.stringify(doc.data());
            var removeEtc = docStringify.replace(/['"{}]+/g, '');
            if (removeEtc.search("profileStatus") != -1) {
                var result = removeEtc.substring(removeEtc.lastIndexOf(":") + 1);
                allStatus.push(result);
            }
    ...
    async suspendProfile(profileToSuspend) {
        console.log(profileToSuspend);
        const q = query(collection(db, "CSIT314/All-Users/UserData"), where("userType", "==", profileToSuspend));
        const queryAns = await getDocs(q);
        var userListSuspend = [];
        queryAns.forEach((doc) => {
            userListSuspend.push(doc.id);
        });

        for (let i = 0; i < userListSuspend.length; i++) {
            const updateRef = doc(db, "CSIT314/All-Users/UserData", userListSuspend[i]);
            await updateDoc(updateRef, {
                userStatus: "Suspended"
            })
        }

        const updateProfileStatus = doc(db, "CSIT314/All-Users/UserTypes", profileToSuspend);
        await updateDoc(updateProfileStatus, {
            profileStatus: "Suspended"
        })

        var message = "All accounts under " + profileToSuspend + " have been suspended";
        window.location.href = "AdminPageProfiles.html";
        return message;

    }

    async createUserProfile(profileName) {
        const profileType = collection(db, "CSIT314/All-Users/UserTypes");
        await setDoc(doc(profileType, profileName), {
            profileStatus: "Active"
        })
        var message = "New profile has been created";
        window.location.href = "AdminPageProfiles.html";
        return (message);
    }

    async deleteProfile(profileType) {
        await deleteDoc(doc(db, "CSIT314/All-Users/UserTypes", profileType));

        const q = query(collection(db, "CSIT314/All-Users/UserData"), where("userType", "==", profileType));
        const queryAns = await getDocs(q);
        var userListDelete = [];
        queryAns.forEach((doc) => {
            userListDelete.push(doc.id);
        });
        for (var i = 0; i < userListDelete.length; i++) {
            await deleteDoc(doc(db, "CSIT314/All-Users/UserData", userListDelete[i]));
        }
        var message = "Profile and all accounts under profile have been deleted";
        window.location.href = "AdminPageProfiles.html";
        return message;
    }
    ...
    async getUserInfo(userEmail){
        const qx = query(collection(db, "CSIT314/All-Users/UserData"), where("Email", "==", userEmail));
        const queryAns = await getDocs(qx);
        var userInfo = "";
        queryAns.forEach(async (doc) => {
            userInfo += JSON.stringify(doc.data());
        });
    ...
    async searchProfile(searchType){
        const qx = query(collection(db, "CSIT314/All-Users/UserData"), where("userType", "==", searchType));
        const queryAns = await getDocs(qx);
        var userList = "";
        queryAns.forEach(async (doc) => {
            userList += JSON.stringify(doc.data()) + "---";
        });
    ...
    async deleteUser(userToDelete){
        await deleteDoc(doc(db, "CSIT314/All-Users/UserData", userToDelete));
        var message = ("User has been deleted");
        return message;
    }

    async searchUser(searchEmail){
        const qx = query(collection(db, "CSIT314/All-Users/UserData"), where("Email", "==", searchEmail));
        const queryAns = await getDocs(qx);
        var userData = "";
        queryAns.forEach(async (doc) => {
