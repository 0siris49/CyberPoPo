...
export class reaController {
    ...
    getREAControllerDisplayName(currentEmail) {
        let initREAEntity = new reaEntity();
        var name = initREAEntity.getREAName(currentEmail);
        return name;
...
document.addEventListener("DOMContentLoaded", async function (e) {
    ...
    if(window.location.href.indexOf('REA.html') > -1){
        ...
        var name = await initREAController.getREAControllerDisplayName(reaEmail);
        document.getElementById("reaName").innerHTML = name;   
    }
